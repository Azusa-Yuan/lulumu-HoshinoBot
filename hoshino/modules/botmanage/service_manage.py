...
@on_command('lssv', aliases=('服务列表', '功能列表'), permission=priv.SUPERUSER, only_to_me=True, shell_like=True)
...
@on_command('enable', aliases=('启用', '开启', '打开'), permission=priv.SUPERUSER, only_to_me=True)
async def enable_service(session: CommandSession):
    await switch_service(session, turn_on=True)

@on_command('disable', aliases=('禁用', '关闭'), permission=priv.SUPERUSER, only_to_me=True)
